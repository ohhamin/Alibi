...
- 기존 대화 기록과 자신이 알고 있는 사실, 소지하거나 확인한 증거를 참고하여 다음 행동을 선택한다.
- 이미 대화에서 확인된 내용을 같은 방식으로 다시 묻지 말고, 필요하면 그 답변을 바탕으로 후속 질문이나 다른 행동을 선택한다.
- 증거가 자신에게 유리하거나 불리한 의미를 가질 경우 그 증거와 연결된 행동을 우선 고려한다.
