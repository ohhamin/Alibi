...
탐정의 수사력과 긴장감은 유지하면서 **용의자 NPC 행동만 경량화**되어 있습니다.
...
- 용의자 행동 선택 LLM 상한: 약 5초
- 상한 초과/OpenAI 오류 시 deterministic fallback

NPC↔NPC 대화도 별도로 최적화되어 있습니다.

- 플레이어가 같은 장소에서 목격하는 NPC↔NPC 대화만 자연어 답변을 생성
- 목격 가능한 NPC 답변은 압축 컨텍스트를 사용하고 약 3.5초 상한 적용
- 플레이어가 볼 수 없는 다른 장소의 NPC↔NPC 대화는 두 번째 LLM 호출을 생략
